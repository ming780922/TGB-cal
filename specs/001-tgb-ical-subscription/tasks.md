...
- [ ] T005 [P] Write `db/migrations/001_initial_schema.sql` with core tables (leagues, divisions, teams, team_divisions, games, team_sync, game_sync) and FTS5 virtual table `teams_fts`
...
- [ ] T012 Implement scrape upsert handler in `apps/worker/src/scrape-api.ts`: apply auth middleware; upsert leagues, divisions, teams, team_divisions, games, and game_sync; invalidate `team_sync` for affected tids; return `{ ok, inserted, updated, new_teams }`
...
- [ ] T016 Implement scraper entry point `packages/scraper/index.js`: call homepage.js to get all division ids; for each division call division.js then api-client.js; accumulate `new_teams` count; output `new_teams` to stdout for GitHub Actions (`console.log('new_teams=' + total)`) and as `GITHUB_OUTPUT`
...
- [ ] T026 [P] [US2] Implement `GET /api/teams/search?q=` route in `apps/web/app/api/teams/search/route.ts`: require `q` param (return 400 if missing); for `q.length >= 2` use FTS5 `SELECT t.tid, t.name, t.last_game_at FROM teams t JOIN teams_fts f ON f.rowid = t.tid WHERE teams_fts MATCH '{q}*' ORDER BY rank LIMIT 20`; for `q.length == 1` use `WHERE t.name LIKE '%{q}%' LIMIT 20`; return `{ results: [...] }`
- [ ] T027 [P] [US2] Create homepage server component `apps/web/app/[locale]/page.tsx`: fetch top 8 teams by `last_game_at DESC` from D1 as hot teams; render page with TeamSearch and HotTeams components
